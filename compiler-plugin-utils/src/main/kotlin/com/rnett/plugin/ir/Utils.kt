...
public val CompilerConfiguration.messageCollector: MessageCollector get() = get(CLIConfigurationKeys.MESSAGE_COLLECTOR_KEY, MessageCollector.NONE)
...
public fun IrGeneratorContext.createIrBuilder(symbol: IrSymbol, startOffset: Int = UNDEFINED_OFFSET, endOffset: Int = UNDEFINED_OFFSET): DeclarationIrBuilder =
 ...
 * Helpful for discovering the type parameters of supertypes.
 */
public fun IrType.raiseToOrNull(classifier: IrClassifierSymbol): IrType? = raiseToOrNull { it.classifierOrNull == classifier }
...
public fun <T : IrMemberAccessExpression<*>> T.withValueArguments(vararg namedArgs: Pair<String, IrExpression?>): T = apply { putValueArguments(*namedArgs) }
...
public fun <T : IrMemberAccessExpression<*>> T.withValueArguments(vararg args: IrExpression?): T = apply { putValueArguments(*args) }

public fun <T : IrMemberAccessExpression<*>> T.withExtensionReceiver(receiver: IrExpression?): T = apply { extensionReceiver = receiver }

public fun <T : IrMemberAccessExpression<*>> T.withDispatchReceiver(receiver: IrExpression?): T = apply { dispatchReceiver = receiver }
...
public fun <T : IrMemberAccessExpression<*>> T.withTypeArguments(vararg namedArgs: Pair<String, IrType?>): T = apply { putTypeArguments(*namedArgs) }
...
public fun <T : IrMemberAccessExpression<*>> T.withTypeArguments(vararg args: IrType?): T = apply { putTypeArguments(*args) }